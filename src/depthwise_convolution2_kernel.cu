...
#pragma unroll
        ...
        cudaStream_t stream)
    {
        ...
        for (auto it = kernel_map.key_cbegin(); it != kernel_map.key_cend(); ++it)
        {
            ...
            size_t const num_grid =
                cuda::ceil_div(n_active_in_volume, shared_mem_size);
            ...
            for (int s = 0; s < num_div; s++)
            {
                ...
                size_t const remainder = n_active_in_volume - offset;
                size_t const curr_num_active = remainder < step ? remainder : step;
                dim3 const grid(cuda::ceil_div(in_nchannel, threads.x),
                                cuda::ceil_div(static_cast<unsigned int>(curr_num_active), threads.y));
    ...
    // default_allocator
    template void
    DepthwiseConvolution2BackwardKernelGPU<float, uint32_t, detail::default_allocator<char>>(
    ...
    // c10_allocator
    template void
    DepthwiseConvolution2BackwardKernelGPU<float, uint32_t, detail::c10_allocator<char>>(
        float const *d_in_feat, float *d_grad_in_feat,
        default_types::size_type const in_nchannel, //
        float const *d_grad_out_feat,
        default_types::size_type const out_nchannel,                             //
